...
export function filterArray(data: Array<{}>, searchFields?: { [field: string]: string | number | boolean | null }): Array<{}> {
...
export function doesRowMatch(row: {}, searchField: string, searchValue: any): boolean {
    ...
    if (isStringSearch) {
        return searchThis.toString().toLowerCase().includes(searchValue.toLowerCase());
    }
    const isBooleanOrNumber = typeof searchValue === 'boolean' || typeof searchValue === 'number';
