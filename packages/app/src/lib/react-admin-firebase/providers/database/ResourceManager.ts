...
import { IFirestoreLogger, getAbsolutePath, log, logWarn, messageTypes, parseFireStoreDocument } from '../../misc';
...
export class ResourceManager {
    ...
    public async TryGetResource(resourceName: string, refresh?: 'REFRESH', collectionQuery?: messageTypes.CollectionQueryType): Promise<IResource> {
        if (refresh) {
    ...
    public GetResource(relativePath: string): IResource {
        ...
        if (!resource) {
            throw new Error(`react-admin-firebase: Can't find resource: "${relativePath}"`);
        }
        return resource;
    }

    public async TryGetResourcePromise(relativePath: string, collectionQuery?: messageTypes.CollectionQueryType): Promise<IResource> {
        ...
        const resource: IResource = this.resources[relativePath];
        if (!resource) {
            throw new Error(`react-admin-firebase: Cant find resource: "${relativePath}"`);
        }
        return resource;
    }

    public async RefreshResource(relativePath: string, collectionQuery: messageTypes.CollectionQueryType | undefined) {
        ...
        resource.list = [];
        newDocs.forEach((d) => resource.list.push(parseFireStoreDocument<IResourceItem>(d)));
    ...
    public async getUserIdentifier(): Promise<string> {
        const identifier = this.options.associateUsersById ? await this.getCurrentUserId() : await this.getCurrentUserEmail();
        return identifier;
    }
