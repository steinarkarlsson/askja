...
export async function GetMany<T extends ra.RaRecord>(
    ...
    const permittedData = options.softDelete ? matches.filter((row) => !row['deleted']) : matches;
    if (options.relativeFilePaths) {
        const data = await Promise.all(permittedData.map((d) => recursivelyMapStorageUrls(fireWrapper, d)));
        return {
            data,
        };
    }
