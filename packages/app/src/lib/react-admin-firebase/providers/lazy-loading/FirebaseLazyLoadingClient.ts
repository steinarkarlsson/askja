...
import { log, messageTypes, parseFireStoreDocument, recursivelyMapStorageUrls } from '../../misc';
...
export class FirebaseLazyLoadingClient {
    ...
    public async apiGetList<T extends ra.RaRecord>(resourceName: string, reactAdminParams: ra.GetListParams): Promise<ra.GetListResult<T>> {
        const r = await this.tryGetResource(resourceName);
        const params = getFullParamsForQuery(reactAdminParams, !!this.options.softDelete);

        log('apiGetListLazy', { resourceName, params });

        const { noPagination, withPagination } = await paramsToQuery(r.collection, params, resourceName, this.client.flogger);
            ...
            const parsedData = await Promise.all(
                data.map(async (doc: any) => {
                    for (let fieldName in doc) {
                        doc[fieldName] = await recursivelyMapStorageUrls(this.client.fireWrapper, doc[fieldName]);
                    }
                    return doc;
                })
            );
    ...
    public async apiGetManyReference(
        ...
        const { withPagination } = await paramsToQuery(r.collection, params, resourceName, this.client.flogger);
    ...
    private async tryGetResource(resourceName: string, collectionQuery?: messageTypes.CollectionQueryType): Promise<IResource> {
