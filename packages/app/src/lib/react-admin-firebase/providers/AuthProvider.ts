import { AuthProvider as RaAuthProvider, UserIdentity } from '../misc/react-admin-models';
import { log, logWarn, logger, retrieveStatusTxt } from '../misc';
...
class AuthClient {
    ...
    public async HandleAuthLogin(params: { username: string; password: string }) {
        ...
        if (username && password) {
            try {
                const user = await this.fireWrapper.authSigninEmailPassword(username, password);
...
export function AuthProvider(firebaseConfig: {}, options: RAFirebaseOptions): ReactAdminFirebaseAuthProvider {
